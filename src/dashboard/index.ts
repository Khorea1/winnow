...
import { resolveDataDir, updateConfig, type RotatorConfig } from '../config'
...
export function registerDashboard(server: http.Server, ctx: { config: { current: RotatorConfig }, health: HealthStore, db: Database.Database, eventLog?: EventLog }) {
  ...
  const poolInterval = setInterval(() => {
    try {
      const now = Date.now()
      const aliveCount = [...health.entries()].filter(([raw, h]: any) => {
  ...
  server.on('request', async (req: IncomingMessage, res: ServerResponse) => {
    try {
      ...
      if (pathname === '/events') {
        ...
        sseClients.add(res)
        const cleanup = () => { sseClients.delete(res); clearInterval(hb) }
      ...
      if (pathname === '/api/validate' && req.method === 'POST') {
        ...
        let custom: any = {}
        if (body.trim()) {
          try { custom = JSON.parse(body) } catch { custom = {} }
        }
      ...
      if (pathname === '/__stats') {
        ...
        const list = [...health.entries()].map(([raw, h]: any) => {
        ...
        }).sort((a: any, b: any) => a.score - b.score).slice(0, 50)
        const alive = [...health.entries()].filter(([raw, h]: any) => {
          if (h.frozenUntil > now || now < h.bannedUntil || h.errors >= maxErrors) return false
          return true
        }).length
        respondJson(res, { total: health.size, alive, top: list, targets: cfgRef.current.targets, retries: cfgRef.current.retries, timeout: cfgRef.current.timeout })
        return
      }
...
async function startValidation(res: ServerResponse, ctx: { cfgRef: { current: RotatorConfig }, sseClients: Set<ServerResponse>, health: HealthStore, db: Database.Database, custom: Partial<ValidationOverrides> }) {
  if (_validationRunning) {
  ...
  try {
    ...
    const proxies = content.split('\n').map((l: string) => l.trim()).filter(Boolean).filter((l: string) => !l.startsWith('#'))
