...
import { insertHealth, loadHealth, removeProxyHealth } from '../db'
...
const FATAL_ERR_CODES = new Set([
  ...
  'ECONNRESET',       // connection reset during handshake/connect phase
  'EADDRNOTAVAIL',
  'ENETUNREACH',
  'EHOSTUNREACH',
...
export function classifyError(e: any): 'fatal' | 'transient' {
  ...
  if (typeof e.socksReply === 'number' && SOCKS_FATAL_REPLIES.has(e.socksReply)) return 'fatal'
...
export function transientBanMs(errors: number, banBaseMs: number, banMultiplier: number, banMaxMs: number): number {
  ...
  const raw = banBaseMs * Math.pow(banMultiplier, k)
  return Math.min(raw, banMaxMs)
}

export function applyFailure(h: HealthEntry, e: any, config: { maxFatalErrors: number; fatalBanMs: number; banBaseMs: number; banMultiplier: number; banMaxMs: number }, now: number) {
...
export class HealthStore extends EventEmitter {
  ...
  constructor(db: Database.Database, config: { pruneAfterMs: number; fatalBanMs: number }, eventLog?: EventLog) {
    ...
    this._maxFrozenMs = Math.min(config.pruneAfterMs, config.fatalBanMs * 3)
    this._load()
  ...
  has(proxy: string) { return this._data.has(proxy) }
  ...
  recordSuccess(proxy: string, target: string | undefined, latency: number, now = Date.now()) {
    ...
    this.dirty(proxy)
    if (target) {
      let te = this.getTarget(proxy, target)
      if (te) {
   ...
   */
  recordFailure(proxy: string, target: string | undefined, err: unknown, config: { maxFatalErrors: number; fatalBanMs: number; banBaseMs: number; banMultiplier: number; banMaxMs: number }, now = Date.now()) {
    const star = ensureStarEntry(this, proxy)
    ...
    if (target) {
      let te = this.getTarget(proxy, target)
      if (te) {
        const teWasBanned = te.bannedUntil > now
        const teWasFrozen = te.frozenUntil > now
  ...
  entries() { return this.allEntries() }
  [Symbol.iterator]() { return this.allEntries()[Symbol.iterator]() }
  ...
  private _score(e: HealthEntry, now: number) { return HealthStore.computeScore(e, now) }
  ...
  private _flush() {
    if (this._deleted.size) {
      for (const p of this._deleted) {
        try { removeProxyHealth(this.db, p) } catch {}
      }
      this._deleted.clear()
    }
