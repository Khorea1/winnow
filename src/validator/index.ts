...
Options:
  ...
  -th, --throttle <n>       Jitter throttle in ms between probes (default: 100)
...
`)
    ...
    if (a === '-t' || a === '--threads') { threads = parseInt(args[++i], 10); continue }
    if (a === '-o' || a === '--output') { output = args[++i]; continue }
    if (a === '--json-output') { jsonOutput = args[++i]; continue }
    if (a === '--mode') { mode = args[++i]; continue }
    if (a === '--base-url' || a === '-b') { baseUrl = args[++i]; continue }
    if (a === '--insecure' || a === '-i') { insecure = true; continue }
    if (a === '--strict-tls' || a === '-s') { strictTLS = true; continue }
    if (a === '--anon-check' || a === '-a') { anonCheck = true; continue }
    if (a === '--throttle' || a === '-th') { throttle = parseInt(args[++i], 10); continue }
    if (!a.startsWith('-')) { proxyFile = a }
  }
  ...
  const onProgress = (res: any, stats: any) => {
