...
import { parseLine, ParsedProxy } from './proxy/dial'
import { createProxyServer } from './proxy/server'
import { registerDashboard } from './dashboard'
import { healthCheckTick } from './proxy/rotator'
...
function load() {
  try {
    const list = fs.readFileSync(config.proxyFile, 'utf8').split('\n').map(parseLine).filter((p): p is ParsedProxy => p !== null)
    const newSet = new Set(list.map(p => p.raw))
    for (const k of health.keys()) if (!newSet.has(k)) health.delete(k)
    for (const p of list) if (!health.has(p.raw)) health.set(p.raw, { errors:0, successes:0, latency: 9999, bannedUntil:0, lastOk:0, fatalErrors:0, frozenUntil:0 })
    proxies = list
...
server.listen(config.port, '0.0.0.0', () => {
  log(`[START v4 JS] :${config.port} | file=${config.proxyFile} | retries=${config.retries} | timeout=${config.timeout}ms | targets=${config.targets}\n[INFO] proxy -> http://127.0.0.1:${config.port} | dashboard http://127.0.0.1:${config.port}/dashboard`)
})
...
try {
  fileWatcher = fs.watch(config.proxyFile, () => load())
} catch (e: any) {
