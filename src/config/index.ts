...
function sanitize(cfg: Record<string, unknown>): RotatorConfig {
  ...
  c.validationBaseUrl = ensureString(c.validationBaseUrl, DEFAULTS.validationBaseUrl, (s) => { try { new URL(s); return true } catch { return false } })
...
export function loadConfig(): RotatorConfig {
  ...
  if (process.env.PROXY_FILE && process.env.PROXY_FILE.trim()) cfg.proxyFile = process.env.PROXY_FILE
  if (process.env.WINNOW_PROXY_FILE && process.env.WINNOW_PROXY_FILE.trim()) cfg.proxyFile = process.env.WINNOW_PROXY_FILE
...
export function updateConfig(patch: Partial<RotatorConfig>): RotatorConfig {
  ...
  } catch { /* vazio abaixo é tratado por sanitize */ }
